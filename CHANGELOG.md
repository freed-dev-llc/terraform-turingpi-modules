...
- **Helper scripts** — low-severity hardening from the script audit (follow-up to #70, tracking #71): `set -e`-safe `attempt` increment in the wipe scripts; `-n` node lists are trimmed and empty elements rejected; BMC slot derivation guards against out-of-range/garbage slots for non-`.73–.76` IPs; `talos-wipe`/`k3s-wipe` no longer treat an empty BMC body as a force-power-off failure (they rely on the power recheck); `k3s-wipe` SSH uses `BatchMode=yes` (no interactive password hang); `cluster-preflight` reports the Talos check as skipped when `talosconfig` is absent; unknown CLI options exit non-zero across the scripts; and `find-armbian-image` validates option values (under `set -u`), backs up an existing autoconfig before overwrite, makes `--list` case-insensitive to match selection, fetches more releases (`per_page=100`), and documents `--hostname` as hint-only.
